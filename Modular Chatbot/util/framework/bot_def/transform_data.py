...
def contains(keywords, rasa_example):
    for each in keywords:
        # print(each)
        if each == rasa_example["text"].lower():
            return True
    return False
def contains_entity(keywords, rasa_entity):
    for each in keywords:
        if rasa_entity["entities"]:
            for entity in rasa_entity["entities"]:
            # print(each)
                if each == entity["value"].lower():
                    return True
    return False
# read file
...
for each in modules:
    for each_intent in rasa_examples:
        ...
        if each["nlu"]["entities"].keys():
            entity_names = list(each["nlu"]["entities"].keys())
            for each_entity in entity_names:
                for each_ent in each_intent["entities"]:
                    if each_entity == each_ent["entity"]:
                        if not contains_entity(each["nlu"]["entities"][each_entity], each_intent):
                            print(each["nlu"]["entities"][each_entity])
                            each["nlu"]["entities"][each_entity].append(each_ent["value"].lower())
                            print(each["nlu"]["entities"][each_entity])
        ...
        if key[0] == each_intent["intent"]:
            ...
            if not contains(each["nlu"]["intents"][key[0]], each_intent):
                ...
                each["nlu"]["intents"][key[0]].append(each_intent["text"].lower())
                new_entry_added = True
